...
export default function Dashboard() {
  ...
  setIsAdmin(admin);
  setIsContentCreator(myProfile?.role === "content_creator");
...
function AdministrationTrialMetrics({ subscriptions = [] }) {
  subscriptions = subscriptions.filter(
    (item) => item.mp_subscription_id !== "content-creator-access"
  );
  const now = Date.now();
  ...
  return (
    <section className="space-y-6">
      ...
      <AdminTable title="Comercios que probaron Gestión">
            ...
            {!subscriptions.length && (
              <tr>
                <td colSpan="5" className="p-8 text-center text-slate-500">
                  Todavía no se iniciaron pruebas de Gestión.
                </td>
              </tr>
            )}
          </tbody>
    ...
    return (
      <div className="min-h-screen bg-slate-100 px-4 py-8">
        <div className="mx-auto max-w-5xl">
          <section className="overflow-hidden rounded-[32px] bg-gradient-to-br from-slate-950 via-blue-900 to-red-600 p-7 text-white shadow-2xl md:p-10">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-red-200">
              Cuenta de producción y auditoría
            </p>
            <h1 className="mt-3 text-3xl font-black md:text-5xl">Hola, Luana</h1>
            <p className="mt-4 max-w-2xl text-blue-100">
              Tenés acceso completo a las herramientas creativas y de gestión de prueba, sin acceso a datos privados de otros comercios.
            </p>
          </section>

          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <button onClick={() => navigate("/studio")} className="rounded-[28px] bg-white p-8 text-left shadow-xl transition hover:-translate-y-1">
              <Sparkles className="h-10 w-10 text-blue-600" />
              <h2 className="mt-5 text-2xl font-black">TusComercios Studio</h2>
              <p className="mt-2 text-slate-600">Imágenes, Reels, carruseles, Brand Kit, Mentor IA, biblioteca e historias.</p>
              <span className="mt-6 inline-block font-black text-blue-700">Abrir Studio →</span>
            </button>
            <button onClick={() => navigate("/administracion")} className="rounded-[28px] bg-white p-8 text-left shadow-xl transition hover:-translate-y-1">
              <Calculator className="h-10 w-10 text-red-600" />
              <h2 className="mt-5 text-2xl font-black">TusComercios Gestión</h2>
              <p className="mt-2 text-slate-600">Probá ventas, stock, caja, clientes, documentos, informes y calculadoras.</p>
              <span className="mt-6 inline-block font-black text-red-700">Abrir Gestión →</span>
            </button>
          </div>

          <button
            onClick={async () => { await supabase.auth.signOut(); navigate("/login"); }}
            className="mt-6 rounded-2xl bg-slate-950 px-6 py-3 font-black text-white"
          >
            Cerrar sesión
          </button>
        </div>
